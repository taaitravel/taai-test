...
const Itinerary = () => {
  ...
  const handleEditSubmit = async (type: ItemType, item: any) => {
    ...
    try {
      ...
      const { error } = await supabase.from('itinerary').update({ [type]: updated, itin_map_locations: newMapLocations } as never).eq('id', itineraryData.id);
      if (error) throw error;
      setEditOpen(false);
      refreshMapData();
      refreshBudgetData();
  ...
  const handleDelete = async (type: string, index: number) => {
    ...
    try {
      ...
      if (item?.from_cart && item?.cart_id) {
        ...
        const { error } = await supabase.from('itinerary').update({ [itemType]: updated } as never).eq('id', itineraryData.id);
        if (error) throw error;
      }
      refreshMapData();
      refreshBudgetData();
