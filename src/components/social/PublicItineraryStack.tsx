...
interface PublicItineraryStackProps {
...
/**
 * Deck-of-cards browser for public (not-yet-yours) itineraries.
 * Mirrors the owned trip stack interaction: swipe ↔ to browse, ↑ to open.
 * Renders card projections only — no itinerary payloads, no queries.
 */
export const PublicItineraryStack: React.FC<PublicItineraryStackProps> = ({ cards }) => {
  ...
  const openTop = () => navigate(`/t/${cards[safeIndex].publicSlug}`);
  ...
  const endDrag = () => {
    ...
    } else if (currentAxis === 'y' && y < -V_THRESHOLD) {
      openTop();
    }
  };

  const visible = Array.from({ length: Math.min(3, len) }, (_, i) => ({
    card: cards[(safeIndex + i) % len],
    i,
  }));
      ...
      >
        {visible
          ...
          .map(({ card, i }) => (
            ...
            >
              <PublicItineraryCard card={card} size="stack" />
            </div>
          ))}
      </div>
...
export default PublicItineraryStack;
