...
export const SubscriptionCard: React.FC<SubscriptionCardProps> = ({
  ...
  return <Card className={`relative p-6 bg-[#171821]/80 border-white/30 backdrop-blur-sm hover:shadow-2xl hover:shadow-white/20 transition-all duration-300 flex flex-col h-full ${isCurrentTier ? 'ring-2 ring-white shadow-lg shadow-white/30' : ''} ${tier.isPopular ? 'border-white/50' : ''}`}>
      ...
      {isCurrentTier && <Badge variant="secondary" className="absolute -top-3 right-4 bg-white/20 text-white border-white/30 mx-0 my-[25px]">
          Current Plan
        </Badge>}
      ...
      <div className="mt-auto">
        {tier.isInquiryOnly ? <Button className="w-full bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white transition-all duration-300" variant="default" onClick={handleSubscribeClick} disabled={loading}>
            <div className="flex items-center gap-2">
              <Mail className="h-4 w-4" />
              Contact Us
            </div>
          </Button> : tier.isPaid ? <Button className={`w-full ${tier.isPopular ? 'gold-gradient hover:opacity-90 text-[#171821] font-semibold' : 'bg-white text-[#171821] border-white hover:bg-gradient-to-r hover:from-[hsl(351,85%,75%)] hover:via-[hsl(15,80%,70%)] hover:to-[hsl(25,75%,65%)] hover:text-white'} transition-all duration-300`} variant={tier.isPopular ? "default" : "outline"} onClick={handleSubscribeClick} disabled={loading || isCurrentTier}>
            {loading ? 'Processing...' : isCurrentTier ? 'Current Plan' : canUpgrade ? 'Subscribe' : 'Change Plan'}
          </Button> : <Button variant="secondary" disabled className="w-full bg-white/10 text-white/70 border-white/20 cursor-not-allowed">
