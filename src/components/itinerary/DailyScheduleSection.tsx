...
export const DailyScheduleSection = ({
  ...
  const buildEventsForDay = (day: Date): EventItem[] => {
    ...
    const flightEvents: EventItem[] = (flights || []).map((f: any) => {
      ...
      return { type: 'flight' as const, title, subtitle, datetime: dt };
    }).filter(e => inDay(e.datetime));

    const hotelCheckin: EventItem[] = (hotels || []).map((h: any) => {
      const dt = safeDate(h?.check_in);
      const title = `Hotel check-in: ${h?.name || ''}`.trim();
      const subtitle = h?.city ? `${h.city}` : undefined;
      return { type: 'hotel-checkin' as const, title, subtitle, datetime: dt };
    }).filter(e => inDay(e.datetime));

    const hotelCheckout: EventItem[] = (hotels || []).map((h: any) => {
      const dt = safeDate(h?.check_out);
      const title = `Hotel check-out: ${h?.name || ''}`.trim();
      const subtitle = h?.city ? `${h.city}` : undefined;
      return { type: 'hotel-checkout' as const, title, subtitle, datetime: dt };
    }).filter(e => inDay(e.datetime));

    const activityEvents: EventItem[] = (activities || []).map((a: any) => {
      const dt = safeDate(a?.date);
      const title = a?.name || 'Activity';
      const subtitle = a?.city ? `${a.city}` : undefined;
      return { type: 'activity' as const, title, subtitle, datetime: dt };
    }).filter(e => inDay(e.datetime));

    const reservationEvents: EventItem[] = (reservations || []).map((r: any) => {
      const dt = fromDateTime(r?.date, r?.time);
      const title = `${r?.type || 'Reservation'}: ${r?.name || ''}`.trim();
      const subtitle = r?.city ? `${r.city}` : undefined;
      return { type: 'reservation' as const, title, subtitle, datetime: dt };
    }).filter(e => inDay(e.datetime));
